...
every known name, formation details, TIN verification. Dumping it raw buries the answer.

The TIN itself is not returned — only whether it verified, the name the IRS holds against it, and
any error. So "what is their EIN" has no answer here; "did their TIN check out" does.
